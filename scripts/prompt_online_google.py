...
RESULTS_BASE_DIR = os.path.join(BASE_DIR, "results_ae", "Online", GOOGLE_MODEL)
    ...
    def generate(self, prompt: str) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                return {"text": text, "raw": {"text": text}, "time_taken": elapsed}
            except Exception as e:
                elapsed = time.time() - start
...
def process_csv_file(csv_file: str, engine: GoogleGenAILLM, run_number: int, subprompts: Dict[str, str]):
...
def main():
    logging.info("Starting Google GenAI online inference...")
    engine = GoogleGenAILLM(api_key=GOOGLE_API_KEY, model=GOOGLE_MODEL)
