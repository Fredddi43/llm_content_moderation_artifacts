...
RESULTS_BASE_DIR = os.path.join(BASE_DIR, "results_ae", "Online", DEEPSEEK_MODEL)
    ...
    def generate(self, prompt: str) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                return {"text": text, "raw": response.model_dump(), "time_taken": elapsed}
            except Exception as e:
                elapsed = time.time() - start
...
def process_csv_file(csv_file: str, engine: DeepSeekLLM, run_number: int, subprompts: Dict[str, str]):
...
def main():
    logging.info("Starting DeepSeek online inference...")
    engine = DeepSeekLLM(api_key=DEEPSEEK_API_KEY, model=DEEPSEEK_MODEL, base_url=DEEPSEEK_API_BASE)
